...
X = X_
for t in range(40):
    y_pred = model.predict(X)    # 预测下一个字符的编号
    print(data_loader.indices_char[np.argmax(y_pred[0])], end='', flush=True)  # 输出预测的字符
    X = np.concatenate([X[:, 1:], [[np.argmax(y_pred[0])]]], axis=-1)     # 将预测的字符接在输入 X 的末尾，并截断 X 的第一个字符，以保证 X 的长度不变
print("\n")